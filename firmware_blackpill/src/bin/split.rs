...
mod app {
    ...
    fn init(cx: init::Context) -> (Shared, Local, init::Monotonics) {
        ...
        // Initialize the monotonic (SysTick rate in STM32F401 is 84MHz)
    ...
    #[task(binds=TIM2, priority=1, local=[debouncer, matrix, timer, serial_tx], shared=[usb_dev, usb_class, layout, is_left_hand])]
    fn tick(mut cx: tick::Context) {
        let is_left_hand = cx.shared.is_left_hand.lock(|val| *val);
        ...
        for event in cx
            ...
            .map(|e| transform_keypress_coordinates(e, is_left_hand))
        {
    ...
    fn transform_keypress_coordinates(e: Event, is_left_hand: bool) -> Event {
